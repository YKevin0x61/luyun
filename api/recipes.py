...
def _validate_tips(items) -> list[str]:
    raw = list(items or [])
    if len(raw) > TIPS_MAX_COUNT:
        raise HTTPException(status_code=400, detail=f"小贴士数量不能超过 {TIPS_MAX_COUNT} 个")
    out: list[str] = []
    for item in raw:
        text = _validate_text(item, "小贴士", required=False, max_len=TIP_TEXT_MAX_LEN)
        if text:
            out.append(text)
    return out

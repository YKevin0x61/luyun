...
describe('pendingKitchenWork', () => {
  it('treats 等叫 as not 待出餐工作', () => {
    ...
    expect(
      isPendingKitchenWork({ dish_status: '待出餐', quantity: 1, is_pending_kitchen_work: true })
    ).toBe(true)
  })

  it('fails closed when the list row was not stamped', () => {
    expect(isPendingKitchenWork({ dish_status: '待出餐', quantity: 1 })).toBe(false)
  })

  it('uses stamped 进入待出餐工作时刻', () => {
    const order = {
  ...
  it('uses stamped 叫起时刻 as 进入待出餐工作时刻', () => {
    const order = {
      order_time: '2026-08-18T10:00:00+08:00',
      fired_at: '2026-08-18T10:20:00+08:00',
      work_enter_time: '2026-08-18T10:20:00+08:00'
    }
    expect(workEnterTimeMs(order)).toBe(new Date(order.work_enter_time).getTime())
  })
  ...
  it('sorts 加急 before non-rush, then by work-enter time', () => {
    ...
    const rushedLate = {
      id: 'b',
      order_time: '2026-08-18T10:30:00+08:00',
      work_enter_time: '2026-08-18T10:30:00+08:00',
      is_rushed: true
    }

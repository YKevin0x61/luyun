...
**别把两条链路混起来**：上面的 `.luyunbak` 包按后端整库快照、**不能互灌**；管理后台的
「导出 DB」（`/api/admin/export/db`，产出 `.db` 文件）则是**两种后端都能用**的业务数据
导出——SQLite 侧走官方 backup API，PG 侧由 `PgConnection.backup` 按表重建（列定义 +
数据，分批读取），导出的 `.db` 可以在任意门店经「导入 DB」逐表合并回去。代价是这条
链路按 `TABLE_DEDUP_KEY` 去重（`orders` 按 `business_flow_id`），同一订单的多行菜品
只进第一行。

...
**管理后台「备份中心 → 导出备份」在 PG 门店照常可用，但不含业务数据**：`.luyunbak`
的业务数据成员是 SQLite 文件，灌不进 PostgreSQL，所以导出面板把「业务数据」置灰；
凭据、运行配置、配方数据与两类卫生照片照常打包。业务数据的离机副本用 `pg_dump`，
命令与整体恢复步骤见 [`deploy/README.md` 第 10.4 节](../../deploy/README.md)。
